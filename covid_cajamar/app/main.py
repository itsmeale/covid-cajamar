...
covid_data_pre_disease_df = pd.read_csv(
    settings.interim_covid_data_pre_disease_dataset["path"]
)

# page header
st.title(
    "Acompanhamento de algumas métricas sobre COVID-19 em cidades do estado de São Paulo"
)

# page sidebar options
city_options = st.sidebar.selectbox(
    "Selecione uma cidade...", list(set(covid_data_df.nome_munic))
)

# filtered dataframes
filtered_covid_data_df = covid_data_df[covid_data_df.nome_munic == city_options]
filtered_covid_data_pre_disease_df = covid_data_pre_disease_df[
    covid_data_pre_disease_df.nome_munic == city_options
]
...
with st.beta_container():
    ...
    with col3:
        st.markdown(
            f"""### Letalidade: {round(filtered_covid_data_df.letalidade.max(), 2)}% """
        )
...
with st.beta_container():
    ...
    linechart = (
        alt.Chart(filtered_covid_data_df)
        .mark_line()
        .encode(
